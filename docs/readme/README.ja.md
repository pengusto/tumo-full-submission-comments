...
バージョン 1.0.4 は Chrome でテスト済みです。その他の Chromium ベースのブラウザ、Firefox、Safari では、このバージョンの実機テストをまだ行っていません。TUMO が Activity Manager のページ構造を変更した場合は、CSS セレクターの更新が必要になることがあります。
